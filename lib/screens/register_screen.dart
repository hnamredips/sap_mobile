...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  final GoogleSignIn _googleSignIn = GoogleSignIn(
  scopes: ['email'],
  clientId: '575550351850-ktdi1a5amhfospkj3ohrkdqdejrkords.apps.googleusercontent.com', // Thêm clientId vào đây
);
...
final GoogleSignInAuthentication googleAuth = await googleUser.authentication;
    ...
    print('Google ID Token: $idToken'); // Theo dõi token trong console
    ...
    if (response.statusCode == 200) {
      print('Google Sign-In API Response: ${response.data}'); // Theo dõi dữ liệu trả về trong console
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
      ...
      print('Google Sign-In API Error: ${response.statusMessage}'); // Theo dõi lỗi trả về trong console
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
    ...
    if (e is DioError) {
      if (e.response != null) {
        errorMessage = 'Error: ${e.response?.statusMessage}';
        print('DioError: ${e.response?.statusMessage}'); // Hiển thị lỗi chi tiết từ Dio
      } else {
        errorMessage = 'Error: ${e.message}';
        print('DioError: ${e.message}'); // Hiển thị lỗi Dio
      }
    }
    print('Error: $e'); // Hiển thị lỗi chung
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
