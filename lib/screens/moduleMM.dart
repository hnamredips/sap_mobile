...
class _ModuleMMState extends State<ModuleMM> {
        ...
        var filteredCertificates = certificateResponse.data['\$values']
            .where((certificate) =>
                    ...
                    certificate['moduleIds']['\$values']
                        .contains(widget.moduleId) // Sử dụng widget.moduleId
                )
            .toList();
        ...
        var filteredTopicAreas = topicAreaResponse.data['\$values']
            .where((topic) =>
                ...
                filteredCertificates
                    .any((cert) => cert['id'] == topic['certificateId']))
            .toList();

        setState(() {
          certificates =
              filteredCertificates; // Chứng chỉ có moduleId = widget.moduleId
          topicAreas =
              filteredTopicAreas; // Lấy topic areas có liên quan đến các chứng chỉ lọc được
          ...
          for (var certificate in certificates) {
            var coursesForCertificate = filteredCourses
                .where((course) => course['certificateId'] == certificate['id'])
                .toList();
            onlineClasses.addAll(coursesForCertificate.where((classData) =>
                classData['mode'] == "online" && classData['status'] == true));
            offlineClasses.addAll(coursesForCertificate.where((classData) =>
  ...
  List<dynamic> getTopicsByCertificateId(int certificateId) {
    return topicAreas
        .where((topic) => topic['certificateId'] == certificateId)
        .toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          '${widget.moduleName} Certificates', // Hiển thị tên module cùng với "Certificates"
          style: TextStyle(
            fontSize: 25,
      ...
      body: isLoading
          ...
          : certificates.isEmpty
              ? Center(
                  child: Text(
                      "No certificates found")) // Thông báo nếu không có chứng chỉ
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return GestureDetector(
                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => CertificateDetail(
                              certificate[
                                  'certificateName'], // Lấy tên chứng chỉ từ API
                              level: certificate['level'] ??
                                  'Intermediate', // Ví dụ cấp độ, lấy từ API
                              idcertificate:
                                  certificate['id'], // Truyền certificateId
                              topics: getTopicsByCertificateId(certificate[
                                  'id']), // Truyền danh sách topic area
                            ),
                          ),
                        );
                      },
                      child: Card(
                        ...
                        elevation: 5, // Thêm hiệu ứng shadow với độ cao 8
                        ...
                        child: Padding(
                          ...
                          child: Text(
                            ...
                            style: TextStyle(
                              color: Colors.black, // Thay đổi màu chữ sang đen
...
class CertificateDetail extends StatelessWidget {
  ...
  final String level;
  final int
      idcertificate; // Thay đổi thành int để sử dụng trong điều hướng EnrollPage
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  ...topics
                      .map(
                          (topic) => CourseItem(courseName: topic['topicName']))
                      .toList(), // Hiển thị các Topic Area
                  SizedBox(height: 10),
                  Text(
                    'Level: $level',
...
class CourseItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      color: Colors.white, // Đặt màu nền của Card
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            Text(
              ...
              style: TextStyle(
                fontSize: 16,
                color: Colors.black, // Đặt màu chữ thành màu trắng để hiển thị rõ trên nền màu
              ),
            ),
          ],
        ),
