...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  Future<void> checkEnrollmentStatus() async {
    ...
    try {
      final enrollmentResponse = await http.get(
          Uri.parse('https://swdsapelearningapi.azurewebsites.net/api/Enrollment/get-all'));
      if (enrollmentResponse.statusCode == 200) {
        ...
        final enrollment = enrollments.firstWhere(
          ...
          orElse: () {
            print("User không có enrollment status 'Success' hoặc không tìm thấy userId phù hợp.");
            return null;
          },
        );
  ...
  Future<void> fetchCourseSessions(int courseId) async {
    try {
      final courseSessionResponse = await http.get(
          Uri.parse('https://swdsapelearningapi.azurewebsites.net/api/CourseSession/get-all'));
      if (courseSessionResponse.statusCode == 200) {
        ...
        setState(() {
          _schedule = sessions.map((session) {
            return {
  ...
  void filterSessionsBySelectedDay() {
    ...
    setState(() {
      filteredSchedule = _schedule
          .where((session) => session['sessionDate'] == selectedDate)
          .toList();
    });
  }
